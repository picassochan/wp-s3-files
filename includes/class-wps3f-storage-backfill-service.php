<?php
/**
 * Background batch backfill for storage classification flags.
 */
...
class WPS3F_Storage_Backfill_Service {
    const CRON_HOOK     = 'wps3f_backfill_storage_flags';
    const OPTION_STATE  = 'wps3f_storage_backfill_state';
    ...
    /**
     * Start a full backfill run from offset 0.
     */
    ...
    /**
     * Retry only failed IDs from the latest run.
     */
    public function retry_failed() {
        ...
        if (!empty($state['failed_ids']) && is_array($state['failed_ids'])) {
            foreach ($state['failed_ids'] as $attachment_id) {
                $ok = $this->offloader->refresh_storage_flags((int) $attachment_id);
                if (!$ok) {
        ...
        $state['failed_ids'] = array_values(array_unique($failed));
        $state['updated_at'] = gmdate('c');
        update_option(self::OPTION_STATE, $state, false);
    }

    /**
     * WP-Cron callback for a single batch backfill.
     */
